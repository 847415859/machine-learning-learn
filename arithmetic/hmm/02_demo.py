...
states = ["cat", "dog"]
id2topic = dict(zip(range(len(states)), states))    # {0: 'cat', 1: 'dog'}
...
vocab2id = dict(zip(vocabulary, range(len(vocabulary))))     # {'tail': 0, 'fetch': 1, 'mouse': 2, 'food': 3}
...
print('generate vocabulary count\n',data)

# pretend this is repeated, so we have more data to learn from:
lengths = [len(X)]*5
sequences = np.tile(data, (5,1))        # 向下平铺5倍的数据
